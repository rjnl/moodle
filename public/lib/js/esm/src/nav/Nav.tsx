...
/**
 * Shared navigation pill engine, used by the primary navigation.
 *
 * @module     core/nav/Nav
 ...
 * @param props.istablist Whether these items belong to an istablist nav's top-level
 ...
 * @param props.istablist Whether the toggle sits inside an `istablist` nav. When true,
 ...
 *                        role="menubar">`. (Plain leaf items are NavPills, which cannot be given a
 *                        role as a prop, so they get the same treatment from stampMenuItemRole.)
...
/**
 * A tab pill rendered for an `istablist` nav (e.g. admin/search.php's site-admin
 * category tabs).
 *
 * @param props Component props.
 ...
 * ("Element has children which are not allowed"), failing the accessibility Behat scenarios on
 * every page with a navbar, and leaving the pills announced as plain links rather than as the
 * menu items core/menu_navigation drives with the arrow keys.
 *
 ...
 */
const MEASURED_CLASS = 'nav-measured';
...
export default function Nav(
    ...
    // Legacy moremenu.js called this unconditionally for every nav, not just istablist
    ...
    //   opening beside the navigation. It observes the container rather than the <ul> so
    ...
    // Matches primarymoremenu.mustache's NonJS fallback: every <li> is role="none" regardless
