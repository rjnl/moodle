...
const keyboardListenerEvents = e => {
    ...
    if (src.classList.contains('dropdown-item')) {
        ...
        // Some consumers (e.g. core/nav/Nav's plain, non-tablist pills) render their
        ...
        if (src.matches(`${SELECTORS.tab},${SELECTORS.menuitem}`) || isPlainMenuListItem) {
            ...
            // menu item or a tab (e.g. within a tablist-style navigation).
 ...
 * core/nav/Nav's overflow "More" toggle is always rendered with role="menuitem" even when it is
 * a child of a role="tablist" list of otherwise role="tab" items. The final any-link/button
 * fallback covers items that carry neither role at all, e.g. core/nav/Nav's plain,
