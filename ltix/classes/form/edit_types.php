...
class edit_types extends moodleform {
    ...
    public function definition() {
        ...
        foreach ($records as $record) {
            $placements[$record->id] = get_string($record->type, 'core_ltix');
        }
        ...
        foreach ($records as $record) {
            ...
            $placementtypefunc = 'add_' . str_replace(':', '_', $record->type) . '_elements';
            if (method_exists($this, $placementtypefunc)) {
                $this->$placementtypefunc($mform);
            } else {
                debugging('Placement configuration method for Placement "' . $record->type . '" not found.', DEBUG_DEVELOPER);
            }
        }
